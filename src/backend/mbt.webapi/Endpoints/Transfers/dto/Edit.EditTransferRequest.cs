...
[PublicAPI]
public class EditTransferRequest : IWithPaidMediaData
{
...
public class EditTransferRequestValidator : AbstractValidator<EditTransferRequest>
{
    public EditTransferRequestValidator()
    {
        ...
        RuleFor(x => x.FromQuarter).NotEmpty().Matches(ValidationRulesConstants.QuarterRegexPattern)
            .WithMessage(ErrorMessages.InvalidQuarter);
        RuleFor(x => x.ToQuarter).NotEmpty().Matches(ValidationRulesConstants.QuarterRegexPattern)
            .WithMessage(ErrorMessages.InvalidQuarter);
        RuleFor(x => x.Amount).GreaterThan(0);
        RuleFor(x => x.Comment).MaximumLength(ValidationRulesConstants.MaxCommentLength);
    }
}
