...
[PublicAPI]
public class CreateBudgetStructureRequest
{
...
[UsedImplicitly]
public class CreateBudgetStructureRequestValidator : AbstractValidator<CreateBudgetStructureRequest>
{
    public CreateBudgetStructureRequestValidator(IApiService apiService,
        IDbBaseRepository<DictionaryDocument> dictionaryRepository, IUserService userService)
    {
        ...
        RuleFor(b => b.BudgetType)
            .NotEmpty()
            .MustAsync((t, token) => BudgetStructureValidators.IsBudgetTypeExists(dictionaryRepository, t, token))
            .WithMessage("Unknown budget type");

        RuleFor(b => b.CostCenter)
            .NotEmpty()
            .MustAsync((c, token) => BudgetStructureValidators.IsCostCenterExists(apiService, c, token))
            .WithMessage("Unknown cost center");

        RuleFor(budget => budget.Status)
            .NotEmpty()
            .Must(status => status == BudgetStatus.InProgress || status == BudgetStatus.ApprovedPlaceholder)
            .WithMessage("Budget status must be 'In progress' or 'Approved Placeholder'");

        RuleFor(budget => budget.Level1)
            .NotEmpty()
            .MustAsync((l, _) => BudgetStructureValidators.IsLevelExists(apiService, l, 1))
            .WithMessage("Level1 doesn't exists");

        RuleFor(budget => budget.Level2)
            .NotEmpty()
            .MustAsync((l, _) => BudgetStructureValidators.IsLevelExists(apiService, l, 2))
            .WithMessage("Level2 doesn't exists");

        RuleFor(budget => budget.Level3)
            .NotEmpty()
            .MustAsync((l, _) => BudgetStructureValidators.IsLevelExists(apiService, l, 3))
            .WithMessage("Level3 doesn't exists");

        RuleFor(budget => budget.OwnerId)
            .NotEmpty()
            .MustAsync((u, t) => BudgetStructureValidators.IsUserExists(userService, u, t))
