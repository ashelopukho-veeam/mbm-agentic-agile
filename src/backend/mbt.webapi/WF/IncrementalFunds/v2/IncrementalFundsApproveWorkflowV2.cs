...
[PublicAPI]
public class IncrementalFundsApproveWorkflowV2 : IWorkflow<IncrementalFundsApproveWorkflowDataV2>
{
    public string Id => WorkflowNames.IncrementalFundsApproveWorkflow;
    public int Version => 2;

    public IStepBuilder<IncrementalFundsApproveWorkflowDataV2, TaskResultStep> GlobalApproveTaskAction(
        IWorkflowBuilder<IncrementalFundsApproveWorkflowDataV2> builder)
    {
    ...
    public IStepBuilder<IncrementalFundsApproveWorkflowDataV2, TaskResultStep> OwnerTaskAction(
        IWorkflowBuilder<IncrementalFundsApproveWorkflowDataV2> builder)
    {
        return builder
    ...
    private Action<IWorkflowBuilder<IncrementalFundsApproveWorkflowDataV2>> _paidMediaTeamApprove = builder =>
    {
        builder
            .StartWith<AddTaskStep>()
            .Input(step => step.AssociatedItemId, data => data.IncrementalFundItem.Id)
            .Input(step => step.Message,
                data =>
                    $@"${data.IncrementalFundItem.Amount} Incremental Fund - Paid Media validation")
            .Input(step => step.Details, data => GetTaskDetailsInfo(data))
            ...
            .Input(s => s.Body, data =>
                MailTemplates.IncrementalFundSubmitted(data.IncrementalFundItem.ToBudget.Title,
                    data.IncrementalFundItem.Amount.ToString(CultureInfo.InvariantCulture)))
            .WaitFor(WorkflowEventNames.ResolveTask,
                data => data.PaidMediaTeamApproverTaskId, cancelCondition: data => data.IsRejected || data.IsSentBack)
    ...
    public void Build(IWorkflowBuilder<IncrementalFundsApproveWorkflowDataV2> builder)
    {
        builder
            .If(data => !string.IsNullOrEmpty(data.PaidMediaTeamApproverId)).Do(_paidMediaTeamApprove)
            .If(data => !data.IsRejected && !data.IsSentBack)
            .Do(b =>
                b.Parallel()
                    .Do(_ => GlobalApproveTaskAction(b))
                    .Do(_ => OwnerTaskAction(b))
                    .Join()
    ...
    private static string GetTaskDetailsInfo(IncrementalFundsApproveWorkflowDataV2 workflowData)
    {
